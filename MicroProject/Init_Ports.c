...
void Port1_Init(void){
  ...
  // Use the port output register to put outputs to an initial state
    ...
    P1DIR &= 0b11101101;//making pins 1 and 4 inputs
    P1REN |= 0b00010010;
    P1OUT |= 0b00010010;//pull up register
}
...
void Port5_Init(void){
  ...
  // Use the port output register to put outputs to an initial state
    ...
    P5DIR |=0b00000111;//output for LEDS on pin 0-2
    P5DIR &=~BIT6;//setting pin3 as input for button

    P5REN |= BIT6;
    P5OUT &= ~BIT6;//pull down resister


    //Making falling edge trigger, interrupt
    P5IE  |= BIT6;//port interrupt enabled
    P5IES |= BIT6;//set to high to low transition
    P5IFG &= ~BIT6;//clear interrupt flag in case was pressed before
    NVIC->ISER[1] |= BIT7; //bit 7 will be 39 since bit 0 is 32
